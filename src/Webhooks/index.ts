...
class Webhooks {
  ...
  createWebhook(data: WebhookParams) {
    return makeRequest<WebhookParams, WebhookResponse>(
      this.path,
      "POST",
      this.secret,
      data
  ...
  retrieveWebhook(id: string) {
    if (!id || id.length === 0) throw "Webhooks id is required";
    return makeRequest<Object, WebhookResponse>(
      `${this.path}/${id}`,
      "GET",
      this.secret,
      {}
